...
void loop() {
  ...
  if(tens==2){
     const char txten2[]="2";radio.write(&transmita,sizeof(transmita));radio.write(&txten2,sizeof(txten2));
  }
  if(tens==1){
     const char txten1[]="1";radio.write(&transmita,sizeof(transmita));radio.write(&txten1,sizeof(txten1));
  }
  if(tens==0){
     const char txten0[]="0";radio.write(&transmita,sizeof(transmita));radio.write(&txten0,sizeof(txten0));
  }
  const char transmitb[]="o";
  if(ones==0){
     const char txone0[]="0";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone0,sizeof(txone0));
  }
  if(ones==1){
     const char txone1[]="1";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone1,sizeof(txone1));
  }
  if(ones==2){
     const char txone2[]="2";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone2,sizeof(txone2));
  }
  if(ones==3){
     const char txone3[]="3";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone3,sizeof(txone3));
  }
  if(ones==4){
     const char txone4[]="4";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone4,sizeof(txone4));
  }
  if(ones==5){
     const char txone5[]="5";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone5,sizeof(txone5));
  }
  if(ones==6){
     const char txone6[]="6";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone6,sizeof(txone6));
  }
  if(ones==7){
     const char txone7[]="7";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone7,sizeof(txone7));
  }
  if(ones==8){
     const char txone8[]="8";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone8,sizeof(txone8));
  }
  if(ones==9){
     const char txone9[]="9";radio.write(&transmitb,sizeof(transmitb));radio.write(&txone9,sizeof(txone9));
  }
  delay(1000);
  counter--;
  if(counter==-1){
