...
const int rs = 12, en = 13, d4 = 8, d5 = 9, d6 = 10, d7 = 11, counterpin=A1,finishpin=A2;
...
bool flag1=0,coinflag=0,finish=0;
char customKey;
const byte ROWS = 4;
const byte COLS = 4;
...
void loop(){ 
  ...
  if(customKey=='A'&&fbit1>0){
      ...
      if(customKey=='A'&&fbit2>0){
        ...
        lcd.clear();lcd.setCursor(0,0);
        rowbuy=rowbuy+1;
        Serial.print("R");Serial.print(rowbuy);Serial.print("Q");Serial.print(quant);
        lcd.print("VENDING");
        for(int i=7;i<=12;i++){
          lcd.setCursor(i,0);
          lcd.print(".");
